...
#BOYS
TRIGGER_KEYWORDS = re.compile(r"\b(good\s?boy|gboy|kys|faggot|goodboy|badboy|slut|whore|fag|son|bad\s?boy)\b", re.IGNORECASE)
...
#MARYAM
TRIGGER_M = re.compile(r"\b(good\s?girl|mommy|shawty|bitch|whore|goodgirl|kys|hoe|slut|faggot|badgirl|shawty|baby|bae|bad\s?girl|blow\s?me|give\s?head)\b", re.IGNORECASE)
TARGET_USER_M = 1269821629614264362
WHITELIST_USER_M = {1305007578857869403, 1212229549459374222, 767662700725403658, 940295461753987132, 713576608136429618, 1369688693337624650, 1003140111258624011, 1196446087972667555, 764189912740134932, 1170371030410350734, 1212398197972926496, 755954707436142593, 1351315291186925660}

h = Homoglyphs()

BANNED_WORDS_PATTERN = re.compile(
    r"""\b(
        r[\W_]*[a@4][\W_]*p[\W_]*[e3][\W_]*[d]                 # raped
        | r[\W_]*[a@4][\W_]*p[\W_]*[i1!|][\W_]*[nɡg69]+         # raping
        | r[\W_]*[a@4][\W_]*p[\W_]*[e3]([ds]?)?                # rape, rapes
        | r[\W_]*[p][\W_]*[e3]                                 # rpe
